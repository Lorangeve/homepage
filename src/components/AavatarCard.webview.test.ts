...
let renderAvatarCardHtml: () => string;
let renderAvatarValidHtml: () => string;

beforeAll(async () => {
  const { module, dispose } = await buildSolidSsrEntryModule<{
    renderAvatarCardHtml: () => string;
    renderAvatarValidHtml: () => string;
  }>(ssrEntry);
...
describe("AvatarCard SSR", () => {
  test("产物含占位与应用名", () => {
    const html = renderAvatarCardHtml();
  ...
  test("合法 URL 含头像图", () => {
    const html = renderAvatarValidHtml();
...
describe("AvatarCard 在 Bun.WebView 中", () => {
  webViewTest("非法 URL 时页面展示占位且含应用名", async () => {
    ...
    await view.navigate(pageDataUrl(renderAvatarCardHtml()));
    expect(
  ...
  webViewTest("合法 URL 时页面展示头像地址", async () => {
    await using view = new Bun.WebView();
    await view.navigate(pageDataUrl(renderAvatarValidHtml()));
    expect(
